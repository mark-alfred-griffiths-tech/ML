...
class LoadH5SavedModel:
    def __init__(self, vae_dir, model_name="winning_vae_dir_models", *args, **kwargs):
        super(LoadH5SavedModel, self).__init__(*args, **kwargs)
        self.conv_model = None
        self.model = None
        self.model_name = model_name
        self.vae_dir = vae_dir
        self.model_name_h5 = str(model_name) + '.h5py'
        self.vae_dir_models = Path(vae_dir.vae_tf_model)
        os.chdir(self.vae_dir_models)  
        self.vae_dir_model_h5_path = Path.home().joinpath(self.vae_dir_models, str(self.model_name_h5))
        self.load_h5_saved_model()
        
    def call(self):
        h5_model = tf.keras.models.load_model(self.model, self.vae_dir_model_h5_path)
        return h5_model
