...
from l1_rmsprop_get_best_hyperparameters import *
...
def get_l1_rmsprop_best_params(l1_rmsprop_best_hps):
    rmsprop_learning_rate = l1_rmsprop_best_hps.get('rmsprop_learning_rate')
    rmsprop_rho = l1_rmsprop_best_hps.get('rmsprop_rho')
    rmsprop_momentum = l1_rmsprop_best_hps.get('rmsprop_momentum')
    rmsprop_epsilon = l1_rmsprop_best_hps.get('rmsprop_epsilon')
    centred = l1_rmsprop_best_hps.get('centred')
    kernel_regularizer_l1 = l1_rmsprop_best_hps.get('kernel_regularizer_l1')
    kernel_regularizer_l2 = l1_rmsprop_best_hps.get('kernel_regularizer_l2')
    bias_regularizer_value = l1_rmsprop_best_hps.get('bias_regularizer_value')
...
def create_l1_rmsprop_model_from_best_hps(bias_regularizer_value, kernel_regularizer_l1, kernel_regularizer_l2,
...
def fit_model(model, data, batch_size, epochs, log_reg_l1_rmsprop_dir):
    my_callbacks = [callbacks.EarlyStopping(monitor='val_loss', mode='min', min_delta=0.00001, patience=4),
                    callbacks.ModelCheckpoint(
                        filepath=str(log_reg_l1_rmsprop_dir.log_reg_l1_rmsprop_param_partial_models) + '/model.{epoch:02d}.h5'),
                   ]
...
class OutputModel:
    def __init__(self, lr_output, model, name, *args, **kwargs):
        ...
        self.model_dir = lr_output.log_reg_l1_rmsprop_param_final_model
        self.model = model
        self.name = name
        self.save_model()
...
def run_parameterised_l1_rmsprop_log_reg():
    ...
    project_name = 'log_reg_tf_l1_rmsprop_hyperbanding_tensorboard'

    log_reg_l1_rmsprop_dir = CreateOutputDirectory(results_dir)
    ...
    #GET BEST PARAMETERS
    l1_rmsprop_best_hps = run_tuner_get_best_hyperparameters_l1_rmsprop(log_reg_l1_rmsprop_dir, project_name, epochs)

    [rmsprop_learning_rate, rmsprop_beta_1, rmsprop_beta_2, rmsprop_epsilon, rmsprop_amsgrad, kernel_regularizer_l1,
     kernel_regularizer_l2, bias_regularizer_value] = get_l1_rmsprop_best_params(l1_rmsprop_best_hps)

    #COMPILE MODEL BASED ON BEST PARAMETERS
    log_reg_l1_rmsprop_model = create_l1_rmsprop_model_from_best_hps(rmsprop_learning_rate, rmsprop_beta_1, rmsprop_beta_2, rmsprop_epsilon, rmsprop_amsgrad,
                                       kernel_regularizer_l1, kernel_regularizer_l2, bias_regularizer_value)

    #FIT MODEL
    log_reg_l1_rmsprop_model = fit_model(log_reg_l1_rmsprop_model, data, batch_size, epochs, log_reg_l1_rmsprop_dir)

    #GET STATISTICAL OUTPUT
    test_results = log_reg_l1_rmsprop_model.evaluate(data.validation_data, verbose=1)
    stutter_preds = log_reg_l1_rmsprop_model.predict(data.validation_data)
    ...
    [mse, bic, aic] = get_mse_bic_aic(np.argmax(data.ytest, axis=1), np.argmax(stutter_preds, axis=1), log_reg_l1_rmsprop_model)
    ...
    best_hps_vals_df = pd.DataFrame(l1_rmsprop_best_hps.values, index=[0]).T
    os.chdir(log_reg_l1_rmsprop_dir.log_reg_l1_rmsprop_param_results)
    ...
    OutputModel(log_reg_l1_rmsprop_dir, log_reg_l1_rmsprop_model, name='log_reg_l1_rmsprop_model')

    time_delta = datetime.now() - start_time

    exit_message = 'LR NN L1 RMSPROP PARAMETERISED RAN SUCCESSFULLY IN: ' + str(time_delta)
    sys.exit(exit_message)


# RUN ALL
run_parameterised_l1_rmsprop_log_reg()
