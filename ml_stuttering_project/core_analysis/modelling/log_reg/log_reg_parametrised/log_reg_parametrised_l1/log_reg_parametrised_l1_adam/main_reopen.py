...
from l1_adam_get_best_hyperparameters import *
...
def get_l1_adam_best_params(l1_adam_best_hps):
    adam_learning_rate = l1_adam_best_hps.get('adam_learning_rate')
    adam_beta_1 = l1_adam_best_hps.get('adam_beta_1')
    adam_beta_2 = l1_adam_best_hps.get('adam_beta_2')
    adam_epsilon = l1_adam_best_hps.get('epsilon')
    adam_amsgrad = l1_adam_best_hps.get('adam_amsgrad')
    kernel_regularizer_l1 = l1_adam_best_hps.get('kernel_regularizer_l1')
    kernel_regularizer_l2 = l1_adam_best_hps.get('kernel_regularizer_l2')
    bias_regularizer_value = l1_adam_best_hps.get('bias_regularizer_value')
    return [adam_learning_rate, adam_beta_1, adam_beta_2, adam_epsilon, adam_amsgrad, kernel_regularizer_l1,
            kernel_regularizer_l2,  bias_regularizer_value]

def create_l1_adam_model_from_best_hps(adam_learning_rate, adam_beta_1, adam_beta_2, adam_epsilon, adam_amsgrad,
...
def fit_model(model, data, batch_size, epochs, log_reg_l1_adam_dir):
    my_callbacks = [callbacks.EarlyStopping(monitor='val_loss', mode='min', min_delta=0.00001, patience=4),
                    callbacks.ModelCheckpoint(
                        filepath=str(log_reg_l1_adam_dir.log_reg_l1_adam_param_partial_models) + '/model.{epoch:02d}.h5'),
                   ]
...
class OutputModel:
    def __init__(self, lr_output, model, name, *args, **kwargs):
        ...
        self.model_dir = lr_output.log_reg_l1_adam_param_final_model
        self.model = model
        self.name = name
        self.save_model()
...
def run_parameterised_l1_adam_log_reg():
    ...
    project_name = 'log_reg_tf_l1_adam_hyperbanding_tensorboard'

    log_reg_l1_adam_dir = CreateOutputDirectory(results_dir)
    ...
    #GET BEST PARAMETERS
    l1_adam_best_hps = run_tuner_get_best_hyperparameters_l1_adam(log_reg_l1_adam_dir, project_name, epochs)

    [adam_learning_rate, adam_beta_1, adam_beta_2, adam_epsilon, adam_amsgrad, kernel_regularizer_l1,
     kernel_regularizer_l2, bias_regularizer_value] = get_l1_adam_best_params(l1_adam_best_hps)

    #COMPILE MODEL BASED ON BEST PARAMETERS
    log_reg_l1_adam_model = create_l1_adam_model_from_best_hps(adam_learning_rate, adam_beta_1, adam_beta_2, adam_epsilon, adam_amsgrad,
                                       kernel_regularizer_l1, kernel_regularizer_l2, bias_regularizer_value)

    #FIT MODEL
    log_reg_l1_adam_model = fit_model(log_reg_l1_adam_model, data, batch_size, epochs, log_reg_l1_adam_dir)

    #GET STATISTICAL OUTPUT
    test_results = log_reg_l1_adam_model.evaluate(data.validation_data, verbose=1)
    stutter_preds = log_reg_l1_adam_model.predict(data.validation_data)
    ...
    [mse, bic, aic] = get_mse_bic_aic(np.argmax(data.ytest, axis=1), np.argmax(stutter_preds, axis=1), log_reg_l1_adam_model)
    ...
    best_hps_vals_df = pd.DataFrame(l1_adam_best_hps.values, index=[0]).T
    os.chdir(log_reg_l1_adam_dir.log_reg_l1_adam_param_results)
    ...
    OutputModel(log_reg_l1_adam_dir, log_reg_l1_adam_model, name='log_reg_l1_adam_model')

    time_delta = datetime.now() - start_time

    exit_message = 'LR NN L1 ADAM PARAMETERISED RAN SUCCESSFULLY IN: ' + str(time_delta)
    sys.exit(exit_message)


# RUN ALL
run_parameterised_l1_adam_log_reg()
