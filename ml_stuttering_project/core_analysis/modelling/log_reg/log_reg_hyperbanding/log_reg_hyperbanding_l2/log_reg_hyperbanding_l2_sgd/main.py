...
class run_tuner_get_best_hyperparameters:
    def __init__(self, data, max_epochs, log_reg_dir, min_delta, *args, **kwargs):
        ...
        self.log_reg_l2_sgd_hyperbanding_pretraining = log_reg_dir.log_reg_l2_sgd_hyperbanding_pretraining
        self.log_reg_l2_sgd_hyperbanding_tensorboard = log_reg_dir.log_reg_l2_sgd_hyperbanding_tensorboard
        self.log_reg_l2_sgd_hyperbanding_partial_models = log_reg_dir.log_reg_l2_sgd_hyperbanding_partial_models
    ...
    def run_tuner(self):
        self.tuner = kt.Hyperband(build_model_l2_sgd,
                                  objective=kt.Objective('val_lr_accuracy',direction='max'),
                                  max_epochs=self.max_epochs,
                                  factor=3,
                                  # distribution_strategy=tf.distribute.MirroredStrategy(),
                                  overwrite=False,
                                  directory=self.log_reg_l2_sgd_hyperbanding_pretraining,
                                  project_name='log_reg_tf_l2_sgd_hyperbanding_tensorboard',
                                  logger=TensorBoardLogger(metrics=["loss","accuracy", "val_accuracy", "val_loss",], logdir=self.log_reg_l2_sgd_hyperbanding_pretraining+"/log_reg_tf_l2_sgd_hyperbanding_tensorboard/hparams"))
        setup_tb(self.tuner)
        tensorflow_board = tf.keras.callbacks.TensorBoard(self.log_reg_l2_sgd_hyperbanding_tensorboard)
        partial_models = tf.keras.callbacks.ModelCheckpoint(filepath=self.log_reg_l2_sgd_hyperbanding_partial_models +
