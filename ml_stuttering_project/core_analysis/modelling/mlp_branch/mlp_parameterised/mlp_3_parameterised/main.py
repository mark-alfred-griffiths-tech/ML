...
class RunTuneGetBestSvmHyperparametersThree:
    def __init__(self, max_epochs, min_delta, batch_size, seed_value, *args, **kwargs):
        ...
        mlp_three_dir = CreateMlpThreeDirectory(results_dir= '/users/k1754828/RESULTS/')
...
def run_mlp_three():
    ...
    data = ReformatData(data, batch_size=14000)
    SetSeed(1234)
    mlp_three_dir = CreateMlpThreeDirectory()

    RunTuneGetBestSvmHyperparametersThree()
    best_hps=run_tuner_get_best_hyperparameters(model_dir=mlp_three_dir.mlp_tf_three_layer_pretraining, project_name=, epochs=10000)
    # USE THE MODEL SAVES AS IF STATEMENTS

    epoch_training_model_path = Path(str(mlp_three_dir.mlp_tf_three_layer_epoch_select_model + '/epoch_training_model'))
    if epoch_training_model_path.exists():
        model = tf.keras.models.load_model(str(mlp_three_dir.mlp_tf_three_layer_epoch_select_model + '/epoch_training_model'))
    else:
        ...
        model.save(str(mlp_three_dir.mlp_tf_three_layer_epoch_select_model + '/epoch_training_model'))
        output_best_epoch(mlp_three_dir, best_epoch)

    epoch_training_model_path = Path(str(mlp_three_dir.mlp_tf_three_layer_final_model + '/final_model'))
    if epoch_training_model_path.exists():
        model = tf.keras.models.load_model(str(mlp_three_dir.mlp_tf_three_layer_final_model + '/final_model'))
        best_epoch = get_best_epoch(mlp_three_dir)
    else:
        model = kt.hypermodel.build(best_hps)
        # Retrain the model
        model.fit(data.xytrain, validation_data=(data.xytest), epochs=best_epoch)
        model.save(str(mlp_three_dir.mlp_tf_three_layer_final_model + '/final_model'))
    ...
    exit_message = 'MLP THREE PARAMETERISED RAN SUCCESSFULLY IN: ' + str(time_delta)
