...
class RunTuneGetBestSvmHyperparametersTwo:
    def __init__(self, max_epochs, min_delta, batch_size, seed_value, *args, **kwargs):
        ...
        svm_dir = CreateSvmTwoDirectory(results_dir='/home/debian/RESULTS')

        self.svm = svm_dir.svm_tf_two_layer
        self.svm_tf_two_layer_partial_models = svm_dir.svm_tf_two_layer_partial_models
        self.svm_tf_two_layer_tensorboard = svm_dir.svm_tf_two_layer_tensorboard
        self.svm_tf_two_layer_pretraining = svm_dir.svm_tf_two_layer_pretraining
        self.run_tuner()
    ...
    def run_tuner(self):
        self.tuner = kt.Hyperband(build_model,
                                  ...
                                  overwrite=False,
                                  directory=self.svm_tf_two_layer_pretraining,
                                  project_name='svm_tf_two_layer_tensorboard',
                                  logger=TensorBoardLogger(metrics=["loss", "accuracy", "val_accuracy", "val_loss", ],
                                                           logdir=self.svm_tf_two_layer_pretraining + "/svm_tf_two_layer_tensorboard/hparams")
                                  )
        setup_tb(self.tuner)
        tensorflow_board = tf.keras.callbacks.TensorBoard(self.svm_tf_two_layer_tensorboard)
        partial_models = tf.keras.callbacks.ModelCheckpoint(filepath=self.svm_tf_two_layer_partial_models +
                                                                     '/model.{epoch:02d}.h5')
...
def run_mlp_two():
    ...
    data = ReformatData(data, batch_size=14000)
    SetSeed(1234)
    svm_two_dir = CreateSvmTwoDirectory(results_dir='/home/debian/RESULTS/')

    RunTuneGetBestSvmHyperparametersTwo(max_epochs=epochs, min_delta=min_delta, batch_size=batch_size, seed_value=1234)

    best_hps = run_tuner_get_best_hyperparameters(model_dir=svm_two_dir.svm_tf_two_layer_pretraining,
                                                  project_name='svm_tf_two_layer_tensorboard', epochs=10000)

    epoch_training_model_path = Path(str(svm_two_dir.svm_tf_two_layer_epoch_select_model + '/svm_tf_two_layer_epoch_training_model'))
    if epoch_training_model_path.exists():
        ...
        model.save(str(svm_two_dir.svm_tf_two_layer_epoch_select_model + '/svm_tf_two_layer_epoch_training_model'))
        output_best_epoch.output_best_epoch(svm_two_dir, best_epoch)
        output_best_epoch(svm_two_dir, best_epoch)
    ...
    if epoch_training_model_path.exists():
        ...
        model.save(str(svm_two_dir.svm_tf_two_layer_final_model + '/svm_tf_two_layer_final_model'))
        get_best_epoch(svm_two_dir)

    RunTuneGetBestSvmHyperparametersTwo()
    best_hps=run_tuner_get_best_hyperparameters(model_dir=svm_two_dir.svm_tf_two_layer_pretraining, project_name='svm_tf_two_layer_tensorboard', epochs=10000)

    epoch_training_model_path = Path(str(svm_two_dir.svm_tf_two_layer_epoch_select_model + '/svm_tf_two_layer_epoch_training_model'))
    if epoch_training_model_path.exists():
        model = tf.keras.models.load_model(str(svm_two_dir.svm_tf_two_layer_epoch_select_model + '/svm_tf_two_layer_epoch_training_model'))
    else:
        model = kt.hypermodel.build(best_hps)
    ...
    epoch_training_model_path = Path(str(svm_two_dir.svm_tf_two_layer_final_model + '/svm_tf_two_layer_final_model'))
    if epoch_training_model_path.exists():
        model = tf.keras.models.load_model(str(svm_two_dir.svm_tf_two_layer_final_model + '/svm_tf_two_layer_final_model'))
    else:
        model = kt.hypermodel.build(best_hps)
        # Retrain the model
    ...
    os.chdir(svm_two_dir.svm_tf_two_layer_results)
    ...
    exit_message = 'SVM TWO LAYER PARAMETERISED RAN SUCCESSFULLY IN: ' + str(time_delta)
