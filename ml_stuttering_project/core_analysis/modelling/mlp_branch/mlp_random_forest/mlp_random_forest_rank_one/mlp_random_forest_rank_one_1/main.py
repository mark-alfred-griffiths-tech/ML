...
def fit_model(model, data, rf_tf_one_dir):
    my_callbacks = [callbacks.EarlyStopping(monitor='accuracy', mode='max', min_delta=0.00001, patience=4),
                    callbacks.TensorBoard(log_dir=rf_tf_one_dir.rf_tf_rank_one_one_layer_tensorboard),
                    ]
...
def tune_run_random_forest_phase_one():
    ...
    rf_one_rank_one_dir = CreateRfOneDirectory(results_dir='/home/debian/RESULTS/')
    ...
    model_rank_one = fit_model(model_rank_one, data, rf_one_rank_one_dir)

    OutputModel(rf_one_rank_one_dir, df_and_nn_model)
    ...
    os.chdir(rf_tf_one_dir.rf_tf_rank_one_one_layer_results)
