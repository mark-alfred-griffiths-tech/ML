...
from create_mlp_three_output_directory import *
...
def create_model():
    ...
    mlp_dir=CreateMlpThreeDirectory(results_dir='/home/debian/RESULTS/')

    best_hps_nn = run_tuner_get_best_hyperparameters(mlp_dir, project_name='mlp_tf_three_layer_tensorboard', epochs=10000)

    inputs = tf.keras.Input(shape=(data.dim_x,))

    x = Dense(best_hps_nn.get('nj_neurons'),
                    kernel_initializer=glorot_uniform_initializer(),
                    bias_initializer='zeros',
                    activation = best_hps_nn.get('nj_neurons_activation'),
                    name='nj_neurons_layer')(inputs)
    ...
    x = Dense(best_hps_nn.get('nl_neurons'),
                    kernel_initializer=glorot_uniform_initializer(),
                    bias_initializer='zeros',
