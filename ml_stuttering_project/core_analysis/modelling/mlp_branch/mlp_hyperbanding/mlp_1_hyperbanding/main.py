...
class RunTuneGetBestMlpOneHyperparameters:
    def __init__(self, max_epochs, min_delta, batch_size, seed_value, *args, **kwargs):
        ...
        mlp_one_dir = CreateMlpOneDirectory(results_dir= '/scratch/users/k1754828/RESULTS/')

        self.xytrain = data.xytrain
        self.xytest = data.xytest
        self.mlp = mlp_one_dir.mlp_tf_one_layer
        self.mlp_tf_one_layer_pretraining = mlp_one_dir.mlp_tf_one_layer_pretraining


        self.mlp_tf_one_layer_partial_models = mlp_one_dir.mlp_tf_one_layer_partial_models
        self.mlp_tf_one_layer_tensorboard = mlp_one_dir.mlp_tf_one_layer_tensorboard
        self.run_tuner()
    ...
    def run_tuner(self):
        self.tuner = kt.Hyperband(build_model,
                                  ...
                                  overwrite=False,
                                  directory=self.mlp_tf_one_layer_pretraining,
                                  project_name='mlp_tf_one_layer_tensorboard',
                                  logger=TensorBoardLogger(metrics=["loss", "accuracy", "val_accuracy", "val_loss", ],
                                                           logdir=self.mlp_tf_one_layer_pretraining + "/mlp_tf_one_layer_tensorboard/hparams")
                                  )
        setup_tb(self.tuner)
        tensorflow_board = tf.keras.callbacks.TensorBoard(self.mlp_tf_one_layer_tensorboard)
        partial_models = tf.keras.callbacks.ModelCheckpoint(filepath=self.mlp_tf_one_layer_partial_models +
                                                                     '/model.{epoch:02d}.h5')
...
def run_mlp_one():
    ...
    SetSeed(1234)
    mlp_one_dir = CreateMlpOneDirectory(results_dir='/scratch/users/k1754828/RESULTS/')
    RunTuneGetBestMlpOneHyperparameters(max_epochs = 14000, min_delta = 1e-5, batch_size = 14000, seed_value = 1234)
    best_hps=run_tuner_get_best_hyperparameters(model_dir=mlp_one_dir.mlp_tf_one_layer_pretraining, project_name='mlp_tf_one_layer_tensorboard', epochs=10000)

    epoch_training_model_path = Path(str(mlp_one_dir.mlp_tf_one_layer_epoch_select_model + '/mlp_tf_layer_one_epoch_training_model'))
    if epoch_training_model_path.exists():
        model = tf.keras.models.load_model(str(mlp_one_dir.mlp_tf_one_layer_epoch_select_model + '/mlp_tf_layer_one_epoch_training_model'))
    else:
        ...
        model.save(str(mlp_one_dir.mlp_tf_one_layer_epoch_select_model + '/mlp_tf_layer_one_epoch_training_model'))
        output_best_epoch(mlp_one_dir, best_epoch)

    epoch_training_model_path = Path(str(mlp_one_dir.mlp_tf_one_layer_final_model + '/mlp_tf_layer_one_final_model'))
    if epoch_training_model_path.exists():
        model = tf.keras.models.load_model(str(mlp_one_dir.mlp_tf_one_layer_final_model + '/mlp_tf_layer_one_final_model'))
        best_epoch = get_best_epoch(mlp_one_dir)
    else:
        model = kt.hypermodel.build(best_hps)
        # Retrain the model
        model.fit(data.xytrain, validation_data=(data.xytest), epochs=best_epoch)
        model.save(str(mlp_one_dir.mlp_tf_one_layer_final_model + '/mlp_tf_layer_one_final_model'))

    data = InstantiateData(mlp_one_dir)
    data = DimXNumCats(data)
    data = ConductSklearnStandardScaling(data)
    data = DimXNumCats(data)
    data = ReformatData(data, batch_size)

    best_hps = run_tuner_get_best_hyperparameters(model_dir=mlp_one_dir.mlp_tf_one_layer_pretraining,
                                                  project_name='mlp_tf_one_layer_tensorboard', epochs=epochs)
    ...
    exit_message = 'MLP ONE HYPERBAND RAN SUCCESSFULLY IN: ' + str(time_delta)
