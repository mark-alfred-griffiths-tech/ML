...
class RunTuneGetBestMlpHyperparametersFour:
    def __init__(self, max_epochs, min_delta, batch_size, seed_value, *args, **kwargs):
        super(RunTuneGetBestMlpHyperparametersFour, self).__init__(*args, **kwargs)
        ...
        mlp_four_dir = CreateMlpFourDirectory(results_dir='/scratch/users/k1754828/RESULTS/')
    ...
    def run_tuner(self):
        self.tuner = kt.Hyperband(build_model,
                                  ...
                                  project_name='mlp_tf_four_layer_tensorboard',
...
def run_mlp_four():
    ...
    RunTuneGetBestMlpHyperparametersFour(max_epochs=epochs, min_delta=0.0001, batch_size=batch_size, seed_value=1234)

    best_hps=run_tuner_get_best_hyperparameters(model_dir=mlp_four_dir.mlp_tf_four_layer_pretraining, project_name='/mlp_tf_four_layer_tensorboard', epochs=10000)

    epoch_training_model_path = Path(str(mlp_four_dir.mlp_tf_four_layer_epoch_select_model + '/mlp_tf_four_layer_epoch_training_model'))
    if epoch_training_model_path.exists():
        model = tf.keras.models.load_model(str(mlp_four_dir.mlp_tf_four_layer_epoch_select_model + '/mlp_tf_four_layer_epoch_training_model'))
    else:
        ...
        model.save(str(mlp_four_dir.mlp_tf_four_layer_epoch_select_model + '/mlp_tf_four_layer_epoch_training_model'))
        output_best_epoch(mlp_four_dir, best_epoch)

    epoch_training_model_path = Path(str(mlp_four_dir.mlp_tf_four_layer_final_model + '/mlp_tf_four_layer_final_model'))
    if epoch_training_model_path.exists():
        model = tf.keras.models.load_model(str(mlp_four_dir.mlp_tf_four_layer_final_model + '/mlp_tf_four_layer_final_model'))
        best_epoch = get_best_epoch(mlp_four_dir)
    else:
        model = kt.hypermodel.build(best_hps)
        # Retrain the model
        model.fit(data.xytrain, validation_data=(data.xytest), epochs=best_epoch)
        model.save(str(mlp_four_dir.mlp_tf_four_layer_final_model + '/mlp_tf_four_layer_final_model'))

    data = InstantiateData(mlp_four_dir)
    data = DimXNumCats(data)
    data = ConductSklearnStandardScaling(data)
    data = ReformatData(data, batch_size)

    best_hps = run_tuner_get_best_hyperparameters(model_dir=mlp_four_dir.mlp_four_dir.mlp_tf_four_layer_pretraining,
                                                  project_name='mlp_tf_four_layer_tensorboard', epochs=epochs)
    ...
    exit_message = 'MLP FOUR HYPERBAND RAN SUCCESSFULLY IN: ' + str(time_delta)
