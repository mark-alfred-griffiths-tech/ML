...
def call_existing_code(kernel_initializer, scale, output_dim, optimizer_momentum_float_value,
    ...
    model.add(Dense(best_hps.get('ni_neurons'),
                    kernel_initializer=glorot_uniform_initializer(),
                    bias_initializer='zeros',
                    name='ni_neurons_layer'))
    model.add(Activation(best_hps.get('ni_neurons_activation')))

    model.add(Dropout(best_hps.get('ni_neurons_dropout_value')))

    model.add(Dense(best_hps.get('nj_neurons'),
                    kernel_initializer=glorot_uniform_initializer(),
                    bias_initializer='zeros',
                    name='nj_neurons_layer'))
    model.add(Activation(best_hps.get('nj_neurons_activation')))

    model.add(Dense(best_hps('nj_neurons'),
                             kernel_initializer=glorot_uniform_initializer(),
                             bias_initializer='zeros',
                             name='nk_neurons_layer'))
    model.add(Activation(best_hps('nk_neurons_activation')))
    model.add(Dropout(best_hps('nk_neurons_dropout_value')))
