...
class RunTuneGetBestSvmHyperparametersOne:
    def __init__(self, max_epochs, min_delta, batch_size, seed_value, *args, **kwargs):
        ...
        svm_dir = CreateSvmRnnOneDirectory(results_dir='/home/debian/RESULTS')
...
def run_mlp_one():
    ...
    rnn_one_dir = CreateRnnOneDirectory(results_dir='/home/debian/RESULTS/')
    svm_rnn_one_dir = CreateSvmRnnOneDirectory(results_dir='/home/debian/RESULTS/')

    RunTuneGetBestSvmHyperparametersOne(max_epochs=epochs, min_delta=min_delta, batch_size=batch_size, seed_value=1234)

    best_hps = run_tuner_get_best_hyperparameters(model_dir=rnn_one_dir.rnn_tf_one_layer_pretraining,
                                                  project_name='rnn_tf_one_layer_tensorboard', epochs=10000)

    epoch_training_model_path = Path(str(svm_rnn_one_dir.svm_rnn_tf_one_layer_epoch_select_model + '/svm_rnn_tf_one_layer_epoch_training_model'))
    if epoch_training_model_path.exists():
        ...
        model.save(str(svm_rnn_one_dir.svm_rnn_tf_one_layer_epoch_select_model + '/svm_rnn_tf_one_layer_epoch_training_model'))
        output_best_epoch(svm_rnn_one_dir, best_epoch)
    ...
    if epoch_training_model_path.exists():
        ...
        model.save(str(svm_rnn_one_dir.svm_rnn_tf_one_layer_final_model + '/svm_rnn_tf_one_layer_final_model'))
        get_best_epoch(svm_rnn_one_dir)

    RunTuneGetBestSvmHyperparametersOne()
    best_hps=run_tuner_get_best_hyperparameters(model_dir=svm_rnn_one_dir.svm_rnn_tf_one_layer_pretraining, project_name='svm_rnn_tf_one_layer_tensorboard', epochs=10000)

    epoch_training_model_path = Path(str(svm_rnn_one_dir.svm_rnn_tf_one_layer_epoch_select_model + '/svm_rnn_tf_one_layer_epoch_training_model'))
    if epoch_training_model_path.exists():
        model = tf.keras.models.load_model(str(svm_rnn_one_dir.svm_rnn_tf_one_layer_epoch_select_model + '/svm_rnn_tf_one_layer_epoch_training_model'))
    else:
        model = kt.hypermodel.build(best_hps)
    ...
    epoch_training_model_path = Path(str(svm_rnn_one_dir.svm_rnn_tf_one_layer_final_model + '/svm_rnn_tf_one_layer_final_model'))
    if epoch_training_model_path.exists():
        model = tf.keras.models.load_model(str(svm_rnn_one_dir.svm_rnn_tf_one_layer_final_model + '/svm_rnn_tf_one_layer_final_model'))
    else:
        model = kt.hypermodel.build(best_hps)
        # Retrain the model
    ...
    os.chdir(svm_rnn_one_dir.svm_rnn_tf_one_layer_results)
    ...
    exit_message = 'SVM RNN ONE LAYER PARAMETERISED RAN SUCCESSFULLY IN: ' + str(time_delta)
