...
def build_model(hp):
    ...
    model = call_existing_code(nk_neurons, nl_neurons,
...
def call_existing_code(nk_neurons, nl_neurons,
                       nk_neurons_activation, nl_neurons_activation,
                       nk_neurons_dropout_value,
                       nl_neurons_dropout_value,
                       nk_neurons_batch_normalisation_momentum,
...
def run_tuner_get_best_hyperparameters(model_dir, project_name, epochs):
    tuner = kt.Hyperband(build_model, objective='accuracy', max_epochs=epochs, factor=3, directory=model_dir.mlp_tf_two_layer_pretraining,
