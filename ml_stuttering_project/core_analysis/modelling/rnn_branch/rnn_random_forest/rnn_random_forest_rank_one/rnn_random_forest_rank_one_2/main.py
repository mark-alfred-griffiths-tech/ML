...
def fit_model(model, data, rf_tf_two_dir):
    my_callbacks = [callbacks.EarlyStopping(monitor='accuracy', mode='max', min_delta=0.00001, patience=4),
                    callbacks.TensorBoard(log_dir=rf_tf_two_dir.rf_tf_rank_one_two_layer_tensorboard),
                    ]
...
def tune_run_random_forest_phase_two():
    ...
    rf_rnn_two_rank_one_dir = CreateRfRnnTwoDirectory(results_dir='/home/debian/RESULTS/')
    ...
    model_rank_one = fit_model(model_rank_one, data, rf_two_rank_one_dir)

    OutputModel(rf_rnn_two_rank_one_dir, model_rank_one)
    ...
    os.chdir(rf_rnn_two_rank_one_dir.rf_tf_rank_one_two_layer_results)
    ...
    exit_message = 'RF RANK ONE TWO LAYER PARAMETERISED RAN SUCCESSFULLY IN: ' + str(time_delta)
